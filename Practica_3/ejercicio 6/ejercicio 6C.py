...
try:
	...
	query = "SELECT country.name,sitio.countrycode,COUNT(sitio.countrycode) FROM sitio INNER JOIN country ON country.code = sitio.countrycode GROUP BY country.name, sitio.countrycode ORDER BY COUNT(sitio.countrycode)"
	cursor.execute(query)
	mobile_records = cursor.fetchall() #resultados en forma de lista
  
	i = 0
	for row in mobile_records:		
		mapa.loc[mapa['NAME'] == row[0], 'sitioWeb'] = row[2]
		mapa.loc[mapa['SOV_A3'] == row[1], 'sitioWeb'] = row[2]
